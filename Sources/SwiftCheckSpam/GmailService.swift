...
class GmailService {
    ...
    private func performRequest<T: Decodable>(request: URLRequest, debug: Bool) async throws -> T {
        ...
        guard (200..<300).contains(httpResponse.statusCode) else {
            do {
                ...
                throw AppError.apiError("Gmail API Error: \(errorResponse.error.message) (Status: \(errorResponse.error.status ?? "N/A"), Code: \(errorResponse.error.code))")
                ...
                throw AppError.apiError("Gmail API Error (Status \(httpResponse.statusCode)): \(errorBody)")
            }
        }
        do {
    ...
    func listSpamMessages(query: String, timeoutSeconds: Int, debug: Bool) async throws -> [GmailMessage] {
                ...
                Task {
                    ...
                    do {
                        repeat {
                            ...
                            guard let url = components.url else { throw AppError.apiError("Invalid URL for listing messages") }
                            
                            let request = makeRequest(url: url)
                            let response: GmailListMessagesResponse = try await performRequest(request: request, debug: debug)

                            if let messagesMetadata = response.messages {
                                totalFetchedMetadata += messagesMetadata.count
                                if debug { print("\rFetched metadata for \(totalFetchedMetadata) messages...", terminator: "") }
                                ...
                                for metaMsg in messagesMetadata {
                                    ...
                                    guard let msgUrl = URL(string: "\(baseURL)/messages/\(metaMsg.id)?format=minimal") else {
                                    ...
                                    Task.detached { // Detached to not block the pagination loop
                                        do {
                                            ...
                                            let fullMsg: GmailMessage = try await self.performRequest(request: msgRequest, debug: debug)
                                            continuation.yield(fullMsg)
                                        } catch {
                                            if debug { print("Error fetching message \(metaMsg.id): \(error)")}
    ...
    // Helper for timeout
    private func withTimeout<T>(seconds: Int, operation: @escaping @Sendable () async throws -> T) async throws -> T {
    ...
    func getSpamCounts(cutoffDateString: String, timeout: Int, debug: Bool) async throws -> [String: Int] {
        var dailyCounts: [String: Int] = [:]
        let query = "after:\(cutoffDateString)"
        if debug { print("Gmail query: \(query)") }

        let messages = try await listSpamMessages(query: query, timeoutSeconds: timeout, debug: debug)
        ...
        for m in messages {
            guard let internalDateStr = m.internalDate, let internalDateMs = Int64(internalDateStr) else {
                if debug { print("Warning: Missing or invalid internalDate for message ID \(m.id)") }
                continue
            }
            if internalDateMs <= 0 {
                if debug { print("Warning: Invalid internalDate (\(internalDateMs)) for message ID \(m.id)") }
                continue
            }
