...
class GmailAuthenticator {
    ...
    private func loadClientSecret() throws {
        guard let fileURL = URL(string: "file://\(FileManager.default.currentDirectoryPath)/\(credentialsFilePath)") else {
        ...
        do {
            ...
            if let web = secret.web {
                ...
                throw AppError.clientSecretInvalid("Missing 'web' or 'installed' object in credentials.")
            }
            if !(self.clientSecret?.redirectURIs.contains(redirectURI) ?? false) {
                 print("Warning: The redirectURI '\(redirectURI)' is not listed in your credentials.json. Please ensure it's added to the Google Cloud Console for your OAuth client.")
            }
        } catch {
            throw AppError.clientSecretInvalid("Failed to load or parse: \(error.localizedDescription)")
        }
    }

    private func loadToken() throws {
        guard let fileURL = URL(string: "file://\(FileManager.default.currentDirectoryPath)/\(tokenFilePath)") else {
        ...
        do {
            ...
            throw AppError.authenticationFailed("Failed to load or parse token.json: \(error.localizedDescription)")
        }
    }
    ...
    private func getTokenFromWeb() async throws -> String {
        ...
        print("Go to the following link in your browser then type the authorization code or allow the callback:")
        ...
        guard let httpResponse = response as? HTTPURLResponse, httpResponse.statusCode == 200 else {
            let errorBody = String(data: data, encoding: .utf8) ?? "Unknown error"
            throw AppError.authenticationFailed("Token exchange failed: \(httpResponse.statusCode) - \(errorBody)")
        }
